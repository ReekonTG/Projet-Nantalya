...
<div class="container py-5">
    <div class="card shadow-lg" style="border-radius: 15px;">
        ...
        <form action="{{ route('roulants.store') }}" method="POST" class="needs-validation" novalidate>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-6">
                    <label for="cout_total" class="form-label">
                        <i class="fas fa-money-bill-alt"></i> Coût Total
                    </label>
                    <input type="number" step="0.01" class="form-control shadow-sm" id="cout_total" name="cout_total">
                </div>
            </div>
            ...
            <div class="d-grid gap-3 mb-4">
                ...
                <a href="{{ route('roulants.list') }}" class="btn btn-primary btn-lg shadow-sm" style="background: linear-gradient(135deg, #6a11cb, #2575fc); border: none;">
                    <i class="fas fa-list"></i> Voir la liste des Roulants
                </a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        // Fonction pour calculer le coût total automatiquement
        document.getElementById('cout_unitaire').addEventListener('input', updateCoutTotal);
        document.getElementById('nature').addEventListener('input', updateCoutTotal);

        function updateCoutTotal() {
            var coutUnitaire = parseFloat(document.getElementById('cout_unitaire').value) || 0;
            var nature = parseFloat(document.getElementById('nature').value) || 0;
            var coutTotal = coutUnitaire * nature;
            
            // Afficher le cout_total dans le champ dédié
