...
class DetenteurInfoController extends Controller
{
    ...
    if ($dernierDetenteur) {
        if (!$dernierDetenteur->date_retour || !$dernierDetenteur->observation) {
            return redirect()->back()->with('error', 'Impossible d\'ajouter un nouveau détenteur tant que la date de retour et l\'observation du dernier détenteur ne sont pas remplies.');
        }
    }
        $request->validate([
            'date' => 'required|date',
        ...
        DetenteurInfo::create([
            'info_id' => $id, // Lier au matériel informatique
            'date' => $request->date,
            'nom' => $request->nom,
    ...
    public function show($id)
    {
        ...
        return view('DetenteurFicheInfo', compact('materiel', 'detenteurs'));
    }

    // Mettre à jour les informations d'un détenteur
